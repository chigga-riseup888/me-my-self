...
- [1. Thermal 温控系统总览](#1-thermal-温控系统总览)
  ...
  - [1.2 温控系统组成](#12-温控系统组成)
  - [1.3 端到端温控控制链](#13-端到端温控控制链)
  - [1.4 温控闭环运行机制](#14-温控闭环运行机制)
  - [1.5 温控执行层与控制对象](#15-温控执行层与控制对象)
  - [1.6 Kernel Thermal 主链与外围机制边界](#16-kernel-thermal-主链与外围机制边界)
  - [1.7 Runtime 架构验证](#17-runtime-架构验证)
- [2. 温度采集层](#2-温度采集层)
...
系统结构以当前源码为设计依据；历史 Runtime 用于验证对应机制曾在实际 v610 系统中运行，历史瞬时值不作为当前产品固定参数。

---

## 1.2 温控系统组成

v610 Thermal 系统由硬件温度源、DTS 配置、Kernel Thermal Framework、Thermal Zone、Governor、Cooling Device、CPU/GPU 控制对象以及 SDK userspace 扩展组件组成。
...
当前 SoC Cooling Map 仅绑定 `soc_target = 85°C`。

四个 Trip 不构成按温度逐级对应 Cooling Level 的映射；当前 DTS 仅在 `soc_target` 上配置 Cooling Map。
...
该 Runtime 对象表明 v610 vendor Thermal 中存在独立的 SoC Thermal polling 执行上下文。当前 New SDK 可见源码尚未定位其创建函数，内部实现不在当前证据范围内。
...
115°C 为 `critical` Trip，且未绑定 `soc_target` Cooling Map。Critical Trip 与普通 Cooling State 不构成逐级关系。当前 shutdown API / PMIC 行为尚未闭环，最终关机路径保持未确认状态。

---

## 1.6 Kernel Thermal 主链与外围机制边界

### Kernel Thermal 主控制链

当前 Kernel Thermal 主控制链为：
...
`core_ctl` 作为平台 CPU Core Control 子系统独立描述；当前证据未确认其为 Thermal Cooling 的直接执行后端。
...
上述交叉证据支撑当前 Thermal 系统级主流程。后续章节按该主流程展开配置、源码、Runtime 和证据边界。
...
该日志表明 Virtual Thermal 对 Physical Thermal Zone 存在初始化依赖。
...
该初始化序列表明，Early stage 的 `Failed to get thermal zone` 不代表最终 Thermal Cooling 初始化失败。
...
SDK Native userspace 不创建 Thermal Zone，而是基于已注册的 Kernel Thermal sysfs 调整 SoC Thermal policy / enable 状态。
...
70°C `soc_threshold` 不定义为独立 Cooling Device 启动档位。
...
当前仅确认该 Trip 的 DTS 定义；vendor hot handler 行为尚未闭环。
...
115°C Critical Trip 的最终关机 API 保持未确认状态。
...
当 `ambient`、`usb`、`wcn` 等 zone 出现接近 lookup table 边界的异常值时，检查项包括：
...
M8280 BBA 中不存在 `cthermald` 不能作为 Thermal 功能缺失的判据。

Kernel Thermal 主链检查项：
